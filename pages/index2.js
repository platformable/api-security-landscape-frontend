...
export default function Homepage2({ data }) {
...
shadowBlur: 5
  ...
  const AdjacentSecuritySolutions = data.values.filter(
    (company, index) =>
...
//
  ...
  const  apiSecurityPureplayANDDiscovery=  data.values.filter(
    (company, index) =>
      company?.parentCategorySlug?.includes(
        "API Security Pureplay") &&
   ...
   const  apiSecurityPureplayANDApiThreat =  data.values.filter(
    (company, index) =>
      company?.parentCategorySlug?.includes(
        "API Security Pureplay") &&
   ...
   const  apiSecurityPureplayANDVulnerability =  data.values.filter(
    (company, index) =>
      company?.parentCategorySlug?.includes(
        "API Security Pureplay") &&
...
const  APItoolingToSupportSecurityActionsANDData=  data.values.filter(
  (company, index) =>
    company?.parentCategorySlug?.includes(
      "API tooling to support security actions") &&
...
const  APItoolingToSupportSecurityActionsANDDataGovernance=  data.values.filter(
  (company, index) =>
    company?.parentCategorySlug?.includes(
      "API tooling to support security actions") &&
...
const  APItoolingToSupportSecurityActionsANDLegal=  data.values.filter(
  (company, index) =>
    company?.parentCategorySlug?.includes(
      "API tooling to support security actions") &&
...
const  APItoolingToSupportSecurityActionsANDLogin=  data.values.filter(
  (company, index) =>
    company?.parentCategorySlug?.includes(
      "API tooling to support security actions") &&
...
const  APItoolingToSupportSecurityActionsANDIdentity=  data.values.filter(
  (company, index) =>
    company?.parentCategorySlug?.includes(
      "API tooling to support security actions") &&
...
const  APItoolingToSupportSecurityActionsANDApiConsulting=  data.values.filter(
  (company, index) =>
    company?.parentCategorySlug?.includes(
      "API tooling to support security actions") &&
...
const EmbeddedAPISecurity = data.values.filter(
  (company, index) =>
    company?.parentCategorySlug?.includes(
      "Embedded API security")
);

const  EmbeddedAPISecurityANDManagement=  data.values.filter(
  (company, index) =>
    company?.parentCategorySlug?.includes(
      "Embedded API security") &&
...
const  EmbeddedAPISecurityANDWeb=  data.values.filter(
  (company, index) =>
    company?.parentCategorySlug?.includes(
      "Embedded API security") &&
    company?.subcategory?.includes("Web Application Firewalls with API security capabilities") 
).sort((a, b) => a.name.localeCompare(b.name))

const  EmbeddedAPISecurityANDVulnerability=  data.values.filter(
  (company, index) =>
    company?.parentCategorySlug?.includes(
      "Embedded API security") &&
