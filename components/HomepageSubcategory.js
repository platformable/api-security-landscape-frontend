        ...
        {filteredCategory
          ? filteredCategory.map((row, index) => {
              ...
              return (
                ...
                >
                  <div   data-bs-toggle="modal" data-bs-target="#exampleModal" data-tip={row.name} data-for="companyName-tooltip" className={router.pathname!=="/zoom" ? "landscape-img border bg-white" : "homepage-landscape-img border bg-white"}>
                   {row.logo === "" || null ? <img src={`https://res.cloudinary.com/platformable/image/upload/v1648805011/apilandscape/GitHub-Logo_g5x9hs.png`} alt=""  className=""/> : <img srcset={`${handleImages(row.logo)} 2x`} alt="" className="" /> }
                  </div>
